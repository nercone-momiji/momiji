...
async def build(response: Response, *, encoder=None, stream_id: int | None = None, max_frame: int | None = None) -> bytes | tuple[bytes, os.PathLike | None]:
    ...
    if protocol == "HTTP/2.0":
        from . import h2
        return h2.serialize_response(response, encoder, stream_id, response_fields(response), max_frame if max_frame is not None else h2.DEFAULT_MAX_FRAME)
